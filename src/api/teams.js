...
export const apiDeleteTeam = async (idTeam) => {
  ...
  try {
    const results = await doDelete(`${TEAMS_URL}/${idTeam}`, null, mockResponse);
    return results;
  }
  catch(e) {
...
export const apiEditTeam = async (idTeam, school) => {
  let mockResponse = isMocking ? {success: true} : undefined;
  try {
    const result = await doPut(`${TEAMS_URL}/${idTeam}`, null, school, mockResponse);
