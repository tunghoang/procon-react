...
export const validateInitShape = (obj) => {
	...
	if (!Number.isInteger(obj.fuelLimits) || obj.fuelLimits <= 0) problems.push("fuelLimits must be a positive integer");
	...
	if (!Number.isInteger(obj.busyThreshold) || !Number.isInteger(obj.jammedThreshold)) {
		problems.push("busyThreshold and jammedThreshold are required integers");
	}
	return problems;
};
